...
#         FILE:  Table.pm
#
#  DESCRIPTION:  NetSDS::DB::Table - CRUD implementation for NetSDS
#
#        NOTES:  ---
...
C<NetSDS::DB::Table> module provides commonly used CRUD functionality for
...
=item B<new([...])>
    ...
    my $object = NetSDS::DB::Table->new(%options);
...
sub new {
	...
	if ( $params{table} ) {
		...
		return $class->error('Table name is not specified to NetSDS::DB::Table');
	}
